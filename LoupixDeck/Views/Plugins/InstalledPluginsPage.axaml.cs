...
namespace LoupixDeck.Views.Plugins;
...
public partial class InstalledPluginsPage : UserControl
{
    public InstalledPluginsPage()
    {
        ...
        DataContextChanged += (_, _) =>
        {
            ...
            // The store installs and removes through the same coordinator; keep the list current.
    ...
    /// <summary>The window this page sits in; dialogs opened from here need it as their parent.</summary>
    private Window Owner => TopLevel.GetTopLevel(this) as Window;

    private void PopulatePluginList()
    {
        if (DataContext is not PluginsWindowViewModel vm || PluginList == null)
            return;
    ...
    private Control BuildPluginListEntry(PluginsWindowViewModel vm, LoadedPlugin plugin)
    {
    ...
    private static bool IsPluginEnabled(PluginsWindowViewModel vm, string id) =>
        vm.Config.EnabledPlugins.Any(e => string.Equals(e, id, System.StringComparison.OrdinalIgnoreCase));

    private async void TogglePlugin(PluginsWindowViewModel vm, string id, bool enabled)
    {
    ...
    {
        if (DataContext is not PluginsWindowViewModel vm)
            return;

        var zipPath = await FileDialogHelper.OpenZipDialog(Owner);
    ...
    private async void OnRemovePluginClick(object sender, RoutedEventArgs e)
    {
        ...
        var confirmed = isOverride
            ? await ConfirmDialogHelper.AskYesNoAsync(Owner, Loc.Tr("Confirm_ResetPluginTitle"),
                Loc.Tr("Confirm_ResetPluginMessage", name, plugin.BundledFallbackVersion))
            : await ConfirmDialogHelper.AskYesNoAsync(Owner, Loc.Tr("Confirm_RemovePluginTitle"),
