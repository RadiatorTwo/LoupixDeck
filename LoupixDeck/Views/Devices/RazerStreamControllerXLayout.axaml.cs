...
public partial class RazerStreamControllerXLayout : UserControl
{
    ...
    // its editor. Every interactive button carries its LoupedeckButton as CommandParameter.
    // e.Handled stops the tap bubbling to OnBackgroundTapped (which clears the selection).
    // This device has only keys — no dials, LEDs or side strips.
    ...
    private void OnButtonDoubleTapped(object sender, TappedEventArgs e)
    {
        if (DataContext is not MainWindowViewModel vm ||
            (sender as Button)?.CommandParameter is not TouchButton touch)
            return;
