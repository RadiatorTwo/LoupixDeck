...
/// </summary>
public sealed class PluginLogger(string pluginId) : IPluginLogger
{
    private readonly string prefix = $"[plugin:{pluginId}]";

    public void Info(string message) => Console.WriteLine($"{prefix} {message}");

    public void Warn(string message) => Console.WriteLine($"{prefix} WARN: {message}");

    public void Error(string message, Exception exception = null)
    {
        Console.WriteLine($"{prefix} ERROR: {message}");
        if (exception != null)
            Console.WriteLine($"{prefix} {exception}");
    }
}
