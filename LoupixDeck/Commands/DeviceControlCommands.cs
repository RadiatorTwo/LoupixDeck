...
public class PlayVideoCommand(IDeviceService deviceService, IExclusiveModeService exclusiveMode) : IExecutableCommand
{
        ...
        try
        {
            ...
            switch (mode)
            {
                ...
                case VideoMode.Grid:
                    ...
                    // slot gets one KeySize-square chunk with no leftover margin.
                    w = cols * device.KeySize;
                    h = rows * device.KeySize;
                    break;
                default: // Full
    ...
    // Splits one full-grid frame (cols*KeySize x rows*KeySize) into individual square tiles and
        ...
        SKBitmap full, int cols, int rows, CancellationToken token)
    {
        ...
        for (var s = 0; s < cols * rows; s++)
        {
            ...
            var col = s % cols;
            var row = s / cols;
            var src = new SKRect(col * keySize, row * keySize, (col * keySize) + keySize, (row * keySize) + keySize);
